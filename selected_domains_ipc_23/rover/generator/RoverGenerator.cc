...
struct Rover{
	...
	void makeVisible(int gp,Map mp){
		...
		if(vi==travs.end()){
			...
			vector<pair<int,int> >::const_iterator it = &travs[w];
		...
		seed(s), probtype(d.probtype)
	{
		...
		if(!accessible_for_soil.empty()){
			for(int i = 0;i<numsoilgoals;){ 
					...
					vector<int>::iterator s = &accessible_for_soil[fi];
		...
		if(!accessible_for_rock.empty()){
			for(int i = 0;i<numrockgoals;){
					...
					vector<int>::iterator s = &accessible_for_rock[fi];
		...
		for(int i = 0;i<numimagegoals;++i){
				...
				vector<pair<int,int> >::iterator f = &image_suitable_rovers[fi];
