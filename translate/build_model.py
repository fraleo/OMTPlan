...
class JoinRule(BuildRule):
    ...
    def validate(self):
        ...
        left_vars = set([v for v in left_args if isinstance(v, int) or isinstance(v, Variable)])
        right_vars = set([v for v in right_args if isinstance(v, int) or isinstance(v, Variable)])
        eff_vars = set([v for v in eff_args if isinstance(v, int) or isinstance(v, Variable)])
...
class ProductRule(BuildRule):
    ...
    def validate(self):
        assert len(self.conditions) >= 2, self
        cond_vars = [set([v for v in cond.args if isinstance(v, int) or isinstance(v, Variable)])
                     for cond in self.conditions]
        all_cond_vars = reduce(set.union, cond_vars)
        eff_vars = set([v for v in self.effect.args
                        if isinstance(v, int) or isinstance(v, Variable)])
...
class Unifier:
    ...
    def _insert_condition(self, rule, cond_index):
        ...
        constant_arguments = [(arg_index, arg)
                              ...
                              if not isinstance(arg, int) and not isinstance(arg, Variable)]
